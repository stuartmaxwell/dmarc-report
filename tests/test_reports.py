...
valid_xml_reports = [
    ...
    "reports/legacy-decimal-version.xml",
...
def test_invalid_report(invalid_report) -> None:
    ...
    with pytest.raises(exceptions.DMARCParseError):
        parser.DMARCParser.parse_file(test_file)
...
def test_valid_report_bytes(valid_report) -> None:
    ...
    report = parser.DMARCParser.parse_bytes(test_file.read_bytes())
...
def test_invalid_report_bytes(invalid_report) -> None:
    ...
    test_file = Path(__file__).parent / invalid_report
    with pytest.raises(exceptions.DMARCParseError):
        parser.DMARCParser.parse_bytes(test_file.read_bytes())
